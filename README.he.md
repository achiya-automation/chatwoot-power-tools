...
צ'יפים למשתנים ותצוגה מקדימה חיה של ההודעה, ממלא מדיה חוזרת לתבנית, משלים תרגומי עברית
חסרים בממשק המקורי, מוסיף אנליטיקת מסירה לקמפיינים, ומלביש את דשבורד הנציגים במראה של
וואטסאפ (טפט צ'אט, בועות עם זנב ושעה בתוך הבועה, מפרידי ימים, אווטארים עגולים, צ'יפי סינון,
שורת כתיבה אחת עם כפתור שליחה עגול, וירוק של וואטסאפ במקום הכחול של Chatwoot — במצב בהיר וכהה).
כל נציג יכול לכבות את המראה בדפדפן שלו עם `localStorage.setItem('cwptWaTheme', 'off')`.
...
- **role במסד נתונים עם הרשאות מינימליות.** `drip_engine` מקבל `SELECT` לקריאה בלבד רק
  לטבלאות שהמודולים שנבחרו צריכים — כולל שמות, טלפונים ואימיילים למסכי אנשי הקשר,
  הרצפים והקמפיינים. עם `sequences`, הכתיבה מוגבלת ל-`contacts.custom_attributes`
  (מצב השיוך) ולשתי עמודות מטמון התבניות ב-`channel_whatsapp`:
  `message_templates` ו-`message_templates_last_updated`. התקנת `dashboard` בלבד מבטלת
  במפורש הרשאות כתיבה אלה. ה-role לא יכול לשנות שם, טלפון, אימייל או שדה אחר באיש הקשר.
...
| שדרוגי דשבורד | `dashboard` | שדרוג מודאל הקמפיין, מדיה חוזרת לתבניות, תיקוני עברית, אנליטיקת קמפיינים ומראה וואטסאפ לדשבורד |
...
`--modules` מתאר את המצב המלא הרצוי, ולא רק תוספת. הרצה מחדש עם תת-קבוצה מוחקת
תיקיות ונכסים ישנים של מודולים שלא נבחרו, כותבת את הבחירה המדויקת אל
`CWPT_ENABLED_MODULES`, ומכבה בצד השרת נתיבי API, כניסות SPA, מיגרציות, הרשאות ולולאות
של מודולים שלא נבחרו. לכן התקנת `import` בלבד אינה מקימה role במסד הנתונים ואינה מריצה
מיגרציות-בעלים; בצמצום מהתקנה קודמת היא גם מבטלת גישה של ה-role לטבלאות/פונקציות הציבוריות
ומאפסת את פרטי החיבור שנמסרים ל-container החלופי. `dashboard` משתמש בצופה הקמפיינים הבנוי מראש של ה-sidecar, אך כל כניסה שאינה
קמפיינים וכל פעולת רצפים נחסמות בשרת. נתונים קיימים בסכמת `drip` לעולם אינם נמחקים בעת צמצום.
אם ה־Git checkout עצמו נמצא ב־`/opt/chatwoot/chatwoot-power-tools`, נכסי הפריסה הנבחרים
נשמרים בתיקיית בת מבודדת `.cwpt-runtime/`; התקנת תת־קבוצה והסרה אינן מוחקות קבצי מקור
tracked מה־checkout.
...
- התקנת Chatwoot **עצמאית** (self-hosted) על Docker Compose v2, על שרת Linux שיש לכם
  אליו גישת root/sudo.
- **Chatwoot 4.17.1 ומעלה.** זו דרישה קשיחה: ניתוח הקמפיינים קורא את טבלת
  `campaign_recipients` המובנית של Chatwoot. המתקין בודק את גרסת image ה-Rails הפעילה
  ויוצא בלי לבצע שינויים בגרסה ישנה, גרסת pre-release או image בלי מספר גרסה. עבור תג image ללא גרסה שאומת ידנית,
  אפשר להעביר את גרסתו האמיתית דרך `CWPT_CHATWOOT_VERSION=X.Y.Z`.
- שכבת ה-`SafeFetch` של Chatwoot חוסמת שליחת webhooks לכתובות
...
- reverse proxy מול Chatwoot: Caddy או nginx מקבלים route אוטומטי; כל אחר (Traefik וכו')
  מקבל קטע קונפיגורציה מוכן-להעתקה. המתקין מסיים ב-`INCOMPLETE` ובקוד שגיאה עד שהכתובת
  הציבורית `/chatwoot-addons/drip-api/health` מחזירה את אותו build, סט מודולים וזהות פריסה
  אקראית של המנוע המקומי. רק אחר כך מתפרסם בלוק הדשבורד החדש, ולכן route חסר או ישן לעולם
  אינו נחשף למשתמשים ואינו מדווח כהצלחה.
