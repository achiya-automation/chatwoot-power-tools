...
export async function setupDb(pool) {
  ...
  await pool.query('ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS assignee_agent_bot_id int');
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS public.agent_bot_inboxes (
    id int PRIMARY KEY, inbox_id int, agent_bot_id int, status int DEFAULT 0)`);
