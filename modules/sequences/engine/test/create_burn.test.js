  ...
  const fn = async (url, opts = {}) => {
    ...
    if (method === 'GET') {
      return {
        ...
        json: async () => ({
          data: found
            ? [{ name: 'promo_x', language: 'he', category: 'MARKETING', components: [{ type: 'BODY', text: 'hi {{1}}' }] }]
            : [],
        }),
      };
    }
...
test('יוצר עותק — GET מקור ואז POST עם השם החדש וה-components', async () => {
  const f = mockFetch();
  const res = await createTemplateCopy('W', 'T', 'promo_x', 'promo_x_burn1', f);
  assert.deepEqual(res, { name: 'promo_x_burn1', id: '99', status: 'PENDING' });
  const post = f.calls.find((c) => c.method === 'POST');
  assert.ok(post, 'נשלחה בקשת POST');
  assert.equal(post.body.name, 'promo_x_burn1');                 // שם חדש
...
test('POST נכשל (שם קיים) → זורק עם הודעת מטא', async () => {
  ...
  await assert.rejects(() => createTemplateCopy('W', 'T', 'promo_x', 'promo_x_burn1', f), /already exists/);
});

test('חסרים creds → זורק לפני כל קריאת רשת', async () => {
  const f = mockFetch();
  await assert.rejects(() => createTemplateCopy('', 'T', 'promo_x', 'promo_x_burn1', f), /WABA|טוקן/);
