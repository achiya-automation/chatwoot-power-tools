-- 010_send_hour.sql — שליחה בשעה מדויקת (Asia/Jerusalem).
-- שלב יכול לשאת send_hour (0..23); ה-engine מצמיד את next_send_at לשעה הזו ביום היעד
-- (nextSendAt → atJerusalemHour, מטפל ב-DST). NULL = השהיה בלבד (התנהגות קודמת).
-- כך תזמון מצטבר מה-Sheet ("אחרי N ימים שעה HH:00") נוחת בדיוק.

ALTER TABLE drip.sequence_steps ADD COLUMN IF NOT EXISTS send_hour int;

-- ── _sequence_json: מחזיר גם send_hour (כדי שה-UI יטען אותו) ──
...
-- ── save_sequence: כמו ב-006, + שמירת send_hour בכל שלב ──
...
BEGIN
  ...
  INSERT INTO drip.sequence_steps
    (sequence_id, step_order, template_name, language, category, delay_days, delay_hours, params, media_url, send_hour)
  SELECT v_id,
