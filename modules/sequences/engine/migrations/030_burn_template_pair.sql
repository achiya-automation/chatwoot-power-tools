-- 030: זוג תבניות שריפה — save/load של רצף מטפלים ב-template_burn
--
-- מגדירים פעם אחת זוג לכל שלב: תבנית נקייה (template_name) + עותק שריפה
-- (template_burn). המנוע מנתב נמענת רוויה לעותק כדי לא לשרוף את הנקייה
-- (reconcile.js — כבר קיים). כאן רק נחשף השדה דרך העורך: save כותב, load מחזיר.
--
-- ⚠️ זו הגדרה חד-פעמית של זוג קבוע — לא יצירה אוטומטית חוזרת של תבניות (זה נאסר
-- ב-14/07, mechanism rotate.js הוסר). אף תבנית חדשה לא נוצרת מכאן.

-- העמודה הוגדרה עד כה ידנית בבננה בוק בלבד; מוודאים קיום בכל התקנה (כולל חדשות).
ALTER TABLE drip.sequence_steps ADD COLUMN IF NOT EXISTS template_burn text;

-- save_sequence — כמו קודם, + כתיבת template_burn (עמודה אחרונה ב-INSERT).
...
BEGIN
  ...
  INSERT INTO drip.sequence_steps
    (sequence_id, step_order, template_name, language, category, delay_days, delay_hours,
     ...
     send_date, repeat_interval, repeat_unit, allowed_dow, template_burn)
  SELECT v_id,
...
-- _sequence_json — כמו קודם, + החזרת template_burn (נכלל אוטומטית ב-to_jsonb(s)).
  ...
  LEFT JOIN LATERAL (
    ...
    FROM (
      SELECT id, step_order, template_name, language, category,
             ...
             repeat_interval, repeat_unit, allowed_dow, template_burn
