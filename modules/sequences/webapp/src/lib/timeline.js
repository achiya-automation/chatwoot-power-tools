...
// "יום ו-2 שעות" / "3 ימים" / "5 שעות" / "" (כשאפס)
...
const REPEAT_SUFFIX = { day: 'ואז כל יום', week: 'ואז כל שבוע', month: 'ואז כל חודש' };

/*
 * קיזוז שלב לתצוגה. מתחשב בשעה-ביום, בתאריך מוחלט ובחזרה:
 *   "מיד" · "כעבור 4 ימים בשעה 17:00" · "בתאריך 25.06 בשעה 09:00" · "… · ואז כל חודש"
 */
export function formatOffset({ days, hours, sendHour, sendDate, repeatUnit } = {}) {
  ...
  const suffix = repeatUnit && REPEAT_SUFFIX[repeatUnit] ? ` · ${REPEAT_SUFFIX[repeatUnit]}` : '';
  return base + suffix;
}

// משך כולל לתצוגה: "מיידי" כשאפס, אחרת "14 ימים"
export function formatDuration({ days, hours }) {
  if (!days && !hours) return 'מיידי';
...
/*
 * formatWhen("2026-07-02 19:00") → "ה׳ 2.7 · 19:00" — יום בשבוע + תאריך + שעה.
 * מציג את מועד השליחה המחושב של שלב שטרם נשלח ("מתי בדיוק יישלח") במקום קיזוז יחסי
 * ("כעבור X ימים"). מקבל מחרוזת "YYYY-MM-DD HH:MM" בשעון ישראל (כפי שה-engine מחזיר),
 * ומחזיר את הקלט כמות-שהוא אם אינו תאריך-שעה תקין (למשל "בקרוב").
 */
export function formatWhen(s) {
  ...
  return `${HEB_DOW[dow]} ${Number(d)}.${Number(mo)} · ${hm}`;
}
