...
- **התזכורת נשלחת רק כשיש חדשות, לא בקצב קבוע.** אזהרה חוזרת רק אם הצריכה טיפסה עוד `QUOTA_WARN_GROWTH_PCT` מהמכסה ועברו `QUOTA_WARN_EVERY_DAYS`; חריגה חוזרת אחרי `QUOTA_OVER_EVERY_DAYS` אם היא גדלה, או מיד כשהלקוח נכנס למדרגת חיוב חדשה. לקוח שיושב באותו נפח לא מקבל דבר, ולקוח שירד מתחת לסף נמחק מה-state כדי שחצייה עתידית תתחיל מדף חדש.
- ⚠️ **זיכרון התזכורות יושב על המארח** (`STATE_HOST`), לא ב-`/app/tmp` שבקונטיינר. `/app/tmp` הוא חלק משכבת הקונטיינר ונמחק בכל בנייה מחדש — עד 09.2026 זה איפס את הזיכרון בכל עדכון של Chatwoot, וכל לקוח מעל הסף קיבל מייל מחדש למחרת.
- `QUOTA_FROM_NAME` קובע את שם השולח שהלקוח רואה (ברירת מחדל: אחיה אוטומציה) במקום כתובת גולמית.
- `ruby test_remind.rb` מריץ בדיקה ללוגיקת התזכורת בלי Rails ובלי שרת.
